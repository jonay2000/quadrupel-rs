...
pub fn start_loop() -> ! {
    ...
    loop {
        ...
        let (ypr1, dyrp, f1, f2) = state.raw_mode.update(accel, gyro, dt);
        if state.count % 10 == 0 {
        ...
        if time_since_last_print > 100000 {
            ...
            let msg = MessageToComputer::StateInformation {
                ...
                dt: dt_filtered,
                motors,
                sensor_ypr: [ypr1.yaw.to_bits(), ypr1.pitch.to_bits(), ypr1.roll.to_bits(), dyrp.map(|dyrp| dyrp.pitch).unwrap_or(FI32::ZERO).to_bits(), dyrp.map(|dyrp| dyrp.roll).unwrap_or(FI32::ZERO).to_bits()],
                input_typr: [
