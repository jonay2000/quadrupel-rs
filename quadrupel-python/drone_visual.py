...
class Drone:
    ...
    def draw(self):
        ...
        drone.pos[0], drone.pos[1] = (-mousex + self.screen_size[0] / 2) / 6, (-mousey + self.screen_size[1] / 2) / 6
        ...
        arms = [np.array([1, 0, 0]),np.array([0, 0, 1]),np.array([-1, 0, 0]),np.array([0, 0, -1]),np.array([0, 0.3, 0])]
    ...
    def convert_to_2d(self,point):
        return [point[0] * (point[2] / 30) + self.screen_size[0] / 2, point[1] * (point[2] / 30) + self.screen_size[1] / 2]

    def plot_line(self,x, y, col=(0, 0, 255)):
        pos = self.rescale(self.convert_to_2d(x), self.screen_size, self.off), self.rescale(self.convert_to_2d(y), self.screen_size, self.off)
